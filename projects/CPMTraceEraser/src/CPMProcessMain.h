    ...
    virtual bool get_cpm_process_port_and_cpmo_name(std::string & port, std::string & name)
    {
                                    ...
                                    ACE_Get_Opt::ARG_REQUIRED) == -1)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMTraceEraserEvents::ev_cant_add_option,
                LM_ERROR,
                "cpm_process_port");
            return false;
        }
            ...
            switch (option)
            {
                ...
                default:
                    EVENT_HANDLER_SYSTEMLOG->handle_event(_SRC,
                                CPMTraceEraserEvents::ev_arg_parsing_failure,
                                LM_ERROR,
                                option);
                    return false;
            }
...
public:
    ...
    virtual bool init_cpm_process()
    {
        ...
        if (!args_->get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMTraceEraserEvents::ev_bad_parameters,
                LM_ERROR,
        ...
        else
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMTraceEraserEvents::ev_tao_layer_init_failure,
                LM_ERROR);
            return false;
        }
...
int ACE_TMAIN_internal(int argc, ACE_TCHAR ** argv)
{
    ...
    if (args.get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
    {
        ...
        if (mutex.acquire(timeout) == -1)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMTraceEraserEvents::ev_server_already_running,
                LM_ERROR,
                "CPMTraceEraser");
            return -1;
        }
        ...
        {
            CPMProcessBoot<CPMTraceEraserAppLayer> cpm_process_boot(&args);
