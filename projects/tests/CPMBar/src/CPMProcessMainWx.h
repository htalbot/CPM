...
public:
    ...
    virtual bool run()
    {
        if (!app_layer_.init_corba_objects())
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMBarEvents::ev_corba_objects_init_failure,
                LM_ERROR);
            return false;
        }
        ...
        CPMBarAppWx * p = new CPMBarAppWx(&app_layer_, show_);
        wxApp::SetInstance( p );
        int argc = args_->argc();
    ...
    virtual int svc()
    {
        if (!app_layer_.run())
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMBarEvents::ev_app_layer_failure,
                LM_ERROR);

            EVENT_HANDLER_SYSTEMLOG->handle_event(_SRC,
                CPMBarEvents::ev_server_stopped,
                LM_INFO,
                "CPMBar");
...
protected:
    ...
    int WinMainInternal(HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
    {
        ...
        if (argv == NULL)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMBarEvents::ev_args_parsing_failure,
                LM_ERROR,
                "CPMBar");
            return -1;
        }
        ...
        ACE::init();
        {
            ...
            // is defined as environment variable: CPMBar_<cpmo_name>_port. This
            ...
            if (args.get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
            {
                ...
                else
                {
                    ...
                    CPMProcessBootWx<CPMBarAppLayer> cpm_process_boot(&args, false /*show*/);
            ...
            else
            {
                ...
                if (args.get_cpmo_name(cpmo_name))
                {
                    ...
                    else
                    {
                        ...
                        if (mutex.acquire(timeout) == -1)
                        {
                            // An instance of CPMBar process is running.\n"));
                        ...
                        else
                        {
                            ...
                            CPMProcessBootWx<CPMBarAppLayer> cpm_process_boot(&cpm_process_args, true /*show*/);
                            if (cpm_process_boot.init_cpm_process())
                            {
                ...
                {
                    cpm_messagebox_wx("As GUI application, CPMBar must be started with 'cpmo_name' argument.\n\n     Syntax: CPMBar --cpmo_name <discriminant>");
                }
            }
        }
        ACE::fini();
        ...
        if (restart)
        {
            ...
            if (args.get_cpmo_name(cpmo_name))
            {
                ...
                if (pid == -1)
                {
                    cpm_messagebox_wx("Failed to spawn process CPMBar");
                }
            }
            else
            {
                cpm_messagebox_wx("Restart of CPMBar failed due to bad args");
            }
        }
