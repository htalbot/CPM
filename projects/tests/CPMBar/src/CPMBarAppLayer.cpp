...
#include "CPMBar/version.h"
#include "CPMBar/CPMBar.evc.h"
...
bool CPMBarAppLayer::init_corba_objects()
{
    try
    {
    ...
    {
        RAISE_EVENT(CPMBarEvents::ev_object_registration_with_ior_table_failure,
                    LM_ERROR,
                    cpmo_id_.c_str());
        return false;
    }
...
bool CPMBarAppLayer::run()
{
    ...
    EVENT_HANDLER_SYSTEMLOG->handle_event(_SRC,
        CPMBarEvents::ev_server_started,
        LM_INFO,
        cpmo_id_.c_str());
...
bool CPMBarAppLayer::objects_register()
{
    try
    {
        //***************************************
    ...
    {
        RAISE_EVENT(CPMBarEvents::ev_object_registration_with_ior_table_failure2,
                    LM_ERROR);
        return false;
    }
...
bool CPMBarAppLayer::objects_init()
{
    try
    {
        if (!cpm_obj_dummy_.init_i())
        {
            RAISE_EVENT(CPMBarEvents::ev_object_init_failure,
                        LM_ERROR);
            return false;
        }
    }
...
void CPMBarAppLayer::terminate_()
{
}
