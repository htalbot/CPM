...
public:
    ...
    virtual bool run()
    {
        ...
        CPMProcWxAppWx * p = new CPMProcWxAppWx(&app_layer_, show_);
        wxApp::SetInstance( p );
        int argc = args_->argc();
...
protected:
    ...
    int WinMainInternal(HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
    {
        ...
        if (argv == NULL)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMProcWxEvents::ev_args_parsing_failure,
                LM_ERROR,
                "CPMProcWx");
            return -1;
        }
        ...
        ACE::init();
        {
            ...
            // is defined as environment variable: <cpmo_name>_port. This
            ...
            if (args.get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
            {
                ...
                else
                {
                    ...
                    CPMProcessBootWx<CPMProcWxAppLayer> cpm_process_boot(&args, false /*show*/);
            ...
            else
            {
                ...
                if (args.get_cpmo_name(cpmo_name))
                {
					...
					std::string cpmo_name_uc = cpmo_name;
                    std::transform(cpmo_name_uc.begin(), cpmo_name_uc.end(), cpmo_name_uc.begin(), ::toupper);

					std::stringstream ss_port_env_var;
                    ss_port_env_var << cpmo_name_uc << "_PORT";
                    ...
                    else
                    {
                        ...
                        if (mutex.acquire(timeout) == -1)
                        {
                            // An instance of CPMProcWx process is running.\n"));
                        ...
                        else
                        {
                            ...
                            CPMProcessBootWx<CPMProcWxAppLayer> cpm_process_boot(&cpm_process_args, true /*show*/);
                            if (cpm_process_boot.init_cpm_process())
                            {
                ...
                {
                    cpm_messagebox_wx("As GUI application, CPMProcWx must be started with 'cpmo_name' argument.\n\n     Syntax: CPMProcWx --cpmo_name <discriminant>");
                }
            }
        }
        ACE::fini();
        ...
        if (restart)
        {
            ...
            if (args.get_cpmo_name(cpmo_name))
            {
                ...
                if (pid == -1)
                {
                    cpm_messagebox_wx("Failed to spawn process CPMProcWx");
                }
            }
            else
            {
                cpm_messagebox_wx("Restart of CPMProcWx failed due to bad args");
            }
        }
