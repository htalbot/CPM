...
public:
    ...
    virtual bool run()
    {
        if (!app_layer_.init_corba_objects())
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMFooEvents::ev_corba_objects_init_failure,
                LM_ERROR);
            return false;
        }
        ...
        CPMFooAppWx * p = new CPMFooAppWx(&app_layer_, show_);
        wxApp::SetInstance( p );
        int argc = args_->argc();
    ...
    virtual int svc()
    {
        if (!app_layer_.run())
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMFooEvents::ev_app_layer_failure,
                LM_ERROR);

            EVENT_HANDLER_SYSTEMLOG->handle_event(_SRC,
                CPMFooEvents::ev_server_stopped,
                LM_INFO,
                "CPMFoo");
...
protected:
    ...
    int WinMainInternal(HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
    {
        ...
        if (argv == NULL)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                CPMFooEvents::ev_args_parsing_failure,
                LM_ERROR,
                "CPMFoo");
            return -1;
        }
        ...
        ACE::init();
        {
            ...
            // is defined as environment variable: CPMFoo_<cpmo_name>_port. This
            ...
            if (args.get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
            {
                ...
                else
                {
                    ...
                    CPMProcessBootWx<CPMFooAppLayer> cpm_process_boot(&args, false /*show*/);
            ...
            else
            {
                ...
                if (args.get_cpmo_name(cpmo_name))
                {
                    ...
                    else
                    {
                        ...
                        if (mutex.acquire(timeout) == -1)
                        {
                            // An instance of CPMFoo process is running.\n"));
                        ...
                        else
                        {
                            ...
                            CPMProcessBootWx<CPMFooAppLayer> cpm_process_boot(&cpm_process_args, true /*show*/);
                            if (cpm_process_boot.init_cpm_process())
                            {
                ...
                {
                    cpm_messagebox_wx("As GUI application, CPMFoo must be started with 'cpmo_name' argument.\n\n     Syntax: CPMFoo --cpmo_name <discriminant>");
                }
            }
        }
        ACE::fini();
        ...
        if (restart)
        {
            ...
            if (args.get_cpmo_name(cpmo_name))
            {
                ...
                if (pid == -1)
                {
                    cpm_messagebox_wx("Failed to spawn process CPMFoo");
                }
            }
            else
            {
                cpm_messagebox_wx("Restart of CPMFoo failed due to bad args");
            }
        }
