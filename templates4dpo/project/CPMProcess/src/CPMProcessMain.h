            ...
            switch (option)
            {
                ...
                default:
                    EVENT_HANDLER_SYSTEMLOG->handle_event(_SRC,
                                XyzEvents::ev_arg_parsing_failure,
                                LM_ERROR,
                                option);
                    return false;
            }
...
public:
    ...
    virtual bool init_cpm_process()
    {
        ...
        else
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                XyzEvents::ev_tao_layer_init_failure,
                LM_ERROR);
            return false;
        }
...
int ACE_TMAIN_internal(int argc, ACE_TCHAR ** argv)
{
    ...
    if (args.get_cpm_process_port_and_cpmo_name(cpm_process_port, cpmo_name))
    {
        ...
        if (mutex.acquire(timeout) == -1)
        {
            EVENT_HANDLER_SYSTEMLOG->handle_event(
                _SRC,
                XyzEvents::ev_server_already_running,
                LM_ERROR,
                "xyz");
            return -1;
        }
        ...
        {
            CPMProcessBoot<XyzAppLayer> cpm_process_boot(&args);
